<template>
  <!--  公告详情页面-->
  <div id="viewer-discuss" class="inform-cont gWidth">
    <div class="inform-list clearfix editor-page">
      <div class="inform-box" style="padding: 0 20px;">
        <!--        公告详情卡片-->
        <div class="announce-cont-box">
          <div class="title clearfix">
            <h2 title="范例_第一次作业">{{ inform.name }}</h2>
          </div>
          <div class="time">
            <div class="lefttime fl">
              <span class="dates fl">{{ inform.time }}</span>
              <div class="fl" style="margin-left: 20px"><img v-bind:src="this.address" class="userava">
                {{ teacher.name }}
              </div>
            </div>
          ...
          <div class="announce-cont">
            <div class="word">
              <div class="p special" v-html="inform.intro"></div>
            </div>
            <div class="annex">
              <ul class="clearfix">
              </ul>
        ...
        <div class="comment-box">
          ...
          <div class="input-click clearfix" v-if="icon">
            ...
            <p @click="icon2 = true;icon = false;word=''">添加评论</p>
          </div>
          <!--学生添加评论-->
...
export default {
  ...
  methods: {
    ...
    deleted(id) {
      this.$axios.post('/start/deleteInformComments', {
        id: id,
      }, {
        headers: {'Authorization': this.$store.state.Authorization}
      }).then(resp => {
    ...
    getComment() {
      this.$axios.post('/start/InformComments', {
    ...
    setComment() {
      if (this.word !== "") {
        this.$axios.post('/start/InformDiscuss', {
    ...
    getHomeworkByStudent() {
      this.$axios.post('/start/informId',
          {
            InformId: this.InformId,
          },
          {
